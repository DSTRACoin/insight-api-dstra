...
PeerSync.prototype.handleBlock = function (info) {
  ...
  // TODO: fix P2P sync bug
  // - Unexpected hangup
  // - TXs in a block are always not registered on DB
  //   - Incoherence in levelDB detected: Input spent not registered
  self.historicSync.start({}, function (err) {
    if (err) {
      var txt = 'ABORTED with error: ' + err.message;
      console.log('[p2p_sync HACK] ' + txt);
    } else {
